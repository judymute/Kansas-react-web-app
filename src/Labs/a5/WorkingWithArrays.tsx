...
function WorkingWithArrays() {
  ...
  const updateTitle = async () => {
    const response = await axios.put(`${API}/${todo.id}/title/${todo.title}`);
  ...
  const updateDescription = async () => {
    await axios.put(`${API}/${todo.id}/description`, { description: todo.description });
    setTodos(todos.map((t) => (t.id === todo.id ? { ...t, description: todo.description } : t)));
  };

  // Function to update todo completed status on the server
  const updateCompleted = async () => {
    await axios.put(`${API}/${todo.id}/completed`, { completed: todo.completed });
